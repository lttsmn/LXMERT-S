...
class GQADataset:
    ...
    def __init__(self, splits: str):
        ...
        for split in self.splits:
            self.data.extend(json.load(open("lxmert_data/gqa/%s.json" % split)))
        ...
        # Answers
        self.ans2label = json.load(open("lxmert_data/gqa/trainval_ans2label.json"))
        self.label2ans = json.load(open("lxmert_data/gqa/trainval_label2ans.json"))
...
class GQABufferLoader():
    ...
    def load_data(self, name, number):
        if name == 'testdev':
            path = "lxmert_data/gqa_imgfeat/gqa_testdev_obj36.tsv"
        else:
            path = "lxmert_data/gqa_imgfeat/vg_gqa_obj36.tsv"
