...
from tasks.sent_retrieval_data import RetrievalDataset_val, RetrievalTorchDataset_val, RetrievalEvaluator
from tasks.sent_retrieval_model import RetrievalModel
...
class Retrieval:
    ...
    def evaluate(self, test_tuple: DataTuple):
        ...
        for i, (sent, feats, boxes, target,image_idx,caption_idx) in enumerate(loader):
            feats=feats.repeat(500,1,1)
            boxes=boxes.repeat(500,1,1)
            feats, boxes= feats.cuda(),boxes.cuda()
            with torch.no_grad():
                logit = self.model(sent, feats,boxes)
                score_matrix[
                        image_idx, caption_idx * 500 : (caption_idx + 1) * 500
                    ] = (logit[:,1].view(-1).cpu().numpy())
                target_matrix[
                        image_idx, caption_idx * 500 : (caption_idx + 1) * 500
                    ] = (target.view(-1).float().cpu().numpy())
                if caption_idx.item() == 1:
                    rank = np.where(
                        (
                            np.argsort(-score_matrix[image_idx])
                            == np.where(target_matrix[image_idx] == 1)[0][0]
                        )
                        == 1
                    )[0][0]
                    rank_matrix[image_idx] = rank

                    rank_matrix_tmp = rank_matrix[: image_idx + 1]
                    ...
                    results.append(np.argsort(-score_matrix[image_idx]).tolist()[:20])
            count += 1
