...
from torch.nn import CrossEntropyLoss, SmoothL1Loss,KLDivLoss
...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None,token_position=None):
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        #将attention_output进行dense之后再经过优化函数例如gelu等优化
...
class BertPooler_f(nn.Module):
    def __init__(self, config):
        super(BertPooler_f, self).__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.activation = nn.Tanh()
...
class BertVisualObjHead(nn.Module):
    ...
    def forward(self, hidden_states,pooled_output):
        hidden_states = self.transform(hidden_states)
        pooled_output_expand=pooled_output.unsqueeze(1).repeat(1, hidden_states.size(1), 1)
        output = {}
        for key in self.visual_losses:
            output[key] = self.decoder_dict[key](hidden_states*pooled_output_expand)
        return output
...
class BertPreTrainingHeads(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        ...
        self.seq_relationship_new = nn.Linear(config.hidden_size, 30522)
        self.activate=GeLU()
        self.LayerNorm = BertLayerNorm(30522, eps=1e-12)
        self.seq_relationship = nn.Linear(config.hidden_size, 2)

    def forward(self, sequence_output, pooled_output):
        prediction_scores = self.predictions(sequence_output)
        seq_relationship_score = self.LayerNorm(self.activate(self.seq_relationship_new(pooled_output)))
        seq_relationship_score_match = self.seq_relationship(pooled_output)
        return prediction_scores, seq_relationship_score ,seq_relationship_score_match
...
class LXRTModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.pooler_f=BertPooler_f(config)
        self.apply(self.init_bert_weights)
        self.word_embeddings = nn.Embedding(config.vocab_size, 2048, padding_idx=0)

    def forward(self, input_ids, token_type_ids=None,token_position=None, attention_mask=None,
        ...
        embedding_output = self.embeddings(input_ids, token_type_ids,token_position)

        (feat,boxes)=visual_feats
        feat_cls=self.word_embeddings(input_ids[:,0]).unsqueeze(1)
        feat=torch.cat((feat_cls,feat),1)
        box_cls=torch.tensor([0.,0.,1.,1.])
        box_cls=box_cls.expand(feat.size(0),1,4).cuda()
        boxes=torch.cat((box_cls,boxes),1)

        # Run LXRT backbone
        lang_feats, visn_feats = self.encoder(
            embedding_output,
            extended_attention_mask,
            visn_feats=(feat,boxes),
            visn_attention_mask=extended_visual_attention_mask)

        pooled_lang = self.pooler(lang_feats)
        pooled_feat=self.pooler_f(visn_feats)
        pooled_output=pooled_lang*pooled_feat
...
class LXRTPretraining(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
                visual_feats=None, pos=None, obj_labels=None, matched_label=None, ans=None,match_label=None,token_position=None):

        (lang_output, visn_output), pooled_output = self.bert(
            input_ids, token_type_ids, token_position,attention_mask,
            visual_feats=(visual_feats, pos),
        )
        visn_output=visn_output[:,1:,:]
        lang_prediction_scores, cross_relationship_score,cross_relationship_score_match = self.cls(lang_output, pooled_output)
        ...
        if masked_lm_labels is not None and self.task_mask_lm:
            ...
            lang_pre,label=lang_prediction_scores.max(2)
            train_correct = (label== masked_lm_labels).sum().float()
            len=(masked_lm_labels!=-1).sum().float()
            if len==0.:
                mlm_acc=0.
            else:
                mlm_acc=torch.div(train_correct,len)
            acces += (mlm_acc.detach(),)

        if match_label is not None and self.task_topic:
            loss_fct_match=nn.BCEWithLogitsLoss()
            cross_relationship_score_input=cross_relationship_score.view(-1, 30522)
            label_input=match_label.view(-1,30522).float()
            topic_loss = loss_fct_match(
                cross_relationship_score_input,
                label_input
            )
            result=torch.add(cross_relationship_score_input,label_input)
            crect=(result>1.5).sum()+(result<0.5).sum()
            acc_match=crect.float()/torch.numel(label_input)
            total_loss +=topic_loss
            losses += (topic_loss.detach(),)
            acces +=(acc_match.detach(),)
        if matched_label is not None and self.task_matched:
            matched_loss = loss_fct(
                cross_relationship_score_match.view(-1, 2),
                matched_label.view(-1)
            )
            total_loss += matched_loss
            losses += (matched_loss.detach(),)

            pred,label=cross_relationship_score_match.max(1)
            correct = (label== matched_label).sum().float()
            len=(matched_label!=-1).sum().float()
            acc_m=correct.float()/len.float()
            acces+=(acc_m.detach(),)
        if obj_labels is not None and self.task_obj_predict:
            ...
            total_visn_loss = 0.
            visn_prediction_scores_dict = self.obj_predict_head(visn_output,pooled_output)
            #visual_losses:obj,fae...
            for key in VISUAL_CONFIG.visual_losses:
                ...
                #------------------------compute acc------------------------
                if key=='obj' or key=='attr':
                    score,pre_label=visn_prediction_scores.max(2)
                    corect=(pre_label==label).sum().float()
                    len=(label!=-1).sum().float()
                    if len==0:
                        acc=0
                    else:
                        acc=corect/len
        ...
        return total_loss, torch.stack(losses).unsqueeze(0), answer_score.detach(),torch.stack(acces).unsqueeze(0)
...
class LXRTFeatureExtraction(BertPreTrainedModel):
    ...
    def forward(self, input_ids,token_position=None, token_type_ids=None, attention_mask=None, visual_feats=None,
                visual_attention_mask=None):

        feat_seq, pooled_output = self.bert(input_ids, token_type_ids,token_position, attention_mask,
